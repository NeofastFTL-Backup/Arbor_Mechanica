...
public class NatureConverter2RecipeCategory implements IRecipeCategory<NatureConverterRecipe2> {
    ...
    public static final RecipeType<NatureConverterRecipe2> NATURA_CONVERTER2_RECIPE_RECIPE_TYPE =
            new RecipeType<>(UID, NatureConverterRecipe2.class);

    private final IDrawable background;
    private final IDrawable icon;

    public NatureConverter2RecipeCategory(IGuiHelper helper) {
        this.background = helper.createDrawable(TEXTURE,0 ,0, 176, 85);
        this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(Blocks.NT_MACHINE2));
    }

    @Override
    public RecipeType<NatureConverterRecipe2> getRecipeType() {
        return NATURA_CONVERTER2_RECIPE_RECIPE_TYPE;
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, NatureConverterRecipe2 recipe, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.CATALYST, 29, 34).addIngredients(recipe.getIngredients().get(1));
