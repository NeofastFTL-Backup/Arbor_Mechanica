...
public class RootedConduitBlockEntity extends BlockEntity {
    ...
    public RootedConduitBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(TileEntities.ROOTED_CONDUIT_BLOCK_ENTITY.get(), pPos, pBlockState);
    }
    ...
    public static void clientTick(Level pLevel, BlockPos pPos, BlockState pState, RootedConduitBlockEntity pBlockEntity) {
    ...
    public static void serverTick(Level pLevel, BlockPos pPos, BlockState pState, RootedConduitBlockEntity pBlockEntity) {
        ++pBlockEntity.tickCount;
        long i = pLevel.getGameTime();
        List<BlockPos> list = pBlockEntity.effectBlocks;
        if (i % 40L == 0L) {
    ...
    private static void updateHunting(RootedConduitBlockEntity pBlockEntity, List<BlockPos> pPositions) {
        ...
        if (!list.isEmpty()) {
            for(Player player : list) {
                if (pPos.closerThan(player.blockPosition(), (double)j)) {
                    player.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED, 260, 2, true, false, true));
                }
            }
    ...
    private static void updateDestroyTarget(Level pLevel, BlockPos pPos, BlockState pState, List<BlockPos> pPositions, RootedConduitBlockEntity pBlockEntity) {
    ...
    private static void updateClientTarget(Level pLevel, BlockPos pPos, RootedConduitBlockEntity pBlockEntity) {
