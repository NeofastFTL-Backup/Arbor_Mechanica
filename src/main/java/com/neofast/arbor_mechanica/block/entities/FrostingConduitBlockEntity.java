...
public class FrostingConduitBlockEntity extends BlockEntity {
    ...
    public FrostingConduitBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(TileEntities.FROSTING_CONDUIT_BLOCK_ENTITY.get(), pPos, pBlockState);
    }
    ...
    public static void clientTick(Level pLevel, BlockPos pPos, BlockState pState, FrostingConduitBlockEntity pBlockEntity) {
    ...
    public static void serverTick(Level pLevel, BlockPos pPos, BlockState pState, FrostingConduitBlockEntity pBlockEntity) {
        ++pBlockEntity.tickCount;
        long i = pLevel.getGameTime();
        List<BlockPos> list = pBlockEntity.effectBlocks;
        if (i % 40L == 0L) {
    ...
    private static void updateHunting(FrostingConduitBlockEntity pBlockEntity, List<BlockPos> pPositions) {
            ...
            for(int k1 = -2; k1 <= 2; ++k1) {
                for(int l1 = -2; l1 <= 2; ++l1) {
                    ...
                    if ((i2 > 1 || l > 1 || i1 > 1) && (j1 == 0 && (l == 2 || i1 == 2) || k1 == 0 && (i2 == 2 || i1 == 2) || l1 == 0 && (i2 == 2 || l == 2))) {
                        ...
                        if (isFrostingConduitFrameBlueIceSnowBlock(blockstate, pLevel, blockpos1, pPos)) {
    ...
    private static void applyEffects(Level pLevel, BlockPos pPos, List<BlockPos> pPositions) {
        ...
        List<Zombie> list = pLevel.getEntitiesOfClass(Zombie.class, aabb);
        if (!list.isEmpty()) {
            for(Zombie zombie : list) {
                if (pPos.closerThan(zombie.blockPosition(), (double)j)) {
                    zombie.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 260, 2, true, false, true));
                }
            }
    ...
    private static boolean isFrostingConduitFrameBlueIceSnowBlock(BlockState state, LevelReader level, BlockPos pos, BlockPos conduit)
    {
    ...
    private static void updateDestroyTarget(Level pLevel, BlockPos pPos, BlockState pState, List<BlockPos> pPositions, FrostingConduitBlockEntity pBlockEntity) {
    ...
    private static void updateClientTarget(Level pLevel, BlockPos pPos, FrostingConduitBlockEntity pBlockEntity) {
